$testFile = Join-Path $PSScriptRoot "src\components\__tests__\ErrorBoundary.test.tsx"
...
if (Test-Path $testFile) {
    Copy-Item $testFile "$testFile.bak" -Force
    $newContent | Set-Content -Path $testFile -Encoding UTF8
    Write-Host "Updated $testFile"
    npm test "src/components/__tests__/ErrorBoundary.test.tsx"
} else {
    Write-Host "Test file not found at: $testFile"
}
